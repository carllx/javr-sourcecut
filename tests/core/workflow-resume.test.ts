...
`;
  ...
  it("Slice 4: enforces global ceiling on duration tolerance for many segments", async () => {
    const workspaceDir = path.join(tmpDir, "ceiling-workspace");
    await fs.mkdir(workspaceDir, { recursive: true });
    ...
    // 10 segments of 10s each = 100s expected duration
    const cutSegments = Array.from({ length: 10 }, (_, i) => ({
      start: i * 20,
      end: i * 20 + 10,
    }));
    ...
    const mockDescriptor: SourceDescriptor = {
      provider: "eporner",
      providerAssetId: "ceiling-test",
      sourceUrl: job.sourceUrl,
      rawTitle: "Ceiling Test",
      declaredPerformers: [],
      renditions: [
        { formatId: "1080p-av1", resolution: "1080p", height: 1080, vcodec: "av1", directUrl: "https://example.com/1080-av1" },
      ],
    ...
    // Duration is 115s (diff 15s > 12s global ceiling, even though 10 * 5s would have been 50s)
    vi.spyOn(selectiveFetchModule, "runSelectiveFetch").mockImplementation(async (params) => {
      await fs.writeFile(params.outputClipPath, "mock-output");
      return {
        outputClipPath: params.outputClipPath,
        ...
        index: {} as any,
        probeResult: {
          format: { filename: params.outputClipPath, duration: "115.0" } as any,
          videoStream: { index: 0, codec: "av1", width: 1920, height: 1080, fps: 30 },
          duration: 115.0,
          isValid: true,
        },
    ...
    ).rejects.toThrow(/Output duration mismatch: expected ~100.000s[\s\S]*diff 15.000s exceeds fixed tolerance 12.000s/);
  });
});
