...
function cookieNames(header) {
  if (!header) return [];
  return header.split(';').map(x => x.trim().split('=', 1)[0]).filter(Boolean).sort();
}
...
async function readExistingProfileCookies() {
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      if (exited) throw new Error(`Profile browser exited before CDP became available (exit=${exitCode}). Another Chrome process may still own the profile.`);
      await sleep(100);
    }
...
function cookieMatches(c, rawUrl) {
...
function cookieHeader(cookies, rawUrl) {
  const map = new Map();
  for (const c of cookies) if (cookieMatches(c, rawUrl)) map.set(c.name, c.value);
  return [...map].map(([k, v]) => `${k}=${v}`).join('; ');
}

function parseLivePage(html) {
  const id = new URL(TARGET).pathname.match(/\/videos\/([^/]+)/i)?.[1] || 'unknown';
  const sourceRe = /<source\b[^>]*?src=["']([^"']+)["'][^>]*?>/gi;
  const qualityRe = /quality=["']([^"']+)["']/i;
  const renditions = [];
  let m;
  while ((m = sourceRe.exec(html))) {
    const tag = m[0];
    const src = m[1].replace(/&amp;/g, '&');
    if (!/^https?:\/\//i.test(src) || !/\.mp4(?:\?|$)/i.test(src)) continue;
    const q = tag.match(qualityRe)?.[1] || 'unknown';
    const n = Number((q.match(/(\d{3,4})/) || [])[1]) || Number.MAX_SAFE_INTEGER;
    renditions.push({ src, quality: q, numericQuality: n });
  }
  renditions.sort((a, b) => a.numericQuality - b.numericQuality);
  return { id, renditions };
}

function ensureImpit() {
  ...
  if (result.error || result.status !== 0) throw new Error(result.error?.message || `temporary impit install exited ${result.status}`);
  return req.resolve('impit');
}
...
async function main() {
  ...
  console.log(`MEDIA_STATUS=${res.status}`);
  console.log(`MEDIA_CONTENT_RANGE=${cr || 'missing'}`);
