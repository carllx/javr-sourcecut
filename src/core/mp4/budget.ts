...
export interface TransferBudgetTrackerOptions {
  ...
  historicalTransferredBytes?: number;
}
...
export class TransferBudgetTracker {
  ...
  readonly historicalTransferredBytes: number;
  ...
  constructor(options: TransferBudgetTrackerOptions) {
    ...
    this.historicalTransferredBytes = Math.max(0, historicalTransferredBytes);
  }
  ...
  get totalActualBytes(): number {
    return this.historicalTransferredBytes + this._currentRunBytes;
  }
